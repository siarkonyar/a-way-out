...
export function ScreenshotEditor() {
  ...
  async function captureSlide(el: HTMLElement, w: number, h: number, scale: number) {
    ...
    try {
      ...
      // Root cause of blank / frame-less exports: html-to-image serializes the
      // node into an SVG <foreignObject> and rasterizes it, but the browser
      // often paints that SVG before the embedded <img> bitmaps (mockup bezel +
      // screenshot, both large PNGs) finish decoding inside the clone. A single
      // warm-up pass isn't reliable when there are several big images.
      //
      // Fix: first make sure every image in the *live* node is fully decoded,
      // then render repeatedly until two consecutive passes are pixel-identical.
      // Once all bitmaps have rasterized, output stops changing — so a stable
      // pair means the capture is complete. Uniform slides (no device) go stable
      // immediately; image-heavy slides take a few passes. Slower, but correct.
      await decodeImages(el);
      await waitForPaint();

      let last = await toPng(el, opts);
      let lastSig = await captureSignature(last);

      for (let attempt = 1; attempt < MAX_CAPTURE_ATTEMPTS; attempt++) {
        await settle(CAPTURE_SETTLE_MS);
        const next = await toPng(el, opts);
        const nextSig = await captureSignature(next);
        if (signaturesMatch(lastSig, nextSig)) return next;
        last = next;
        lastSig = nextSig;
      }
      // Ran out of attempts; return the freshest pass as best effort.
      return last;
    } finally {
...
// ---------- Capture reliability tuning ----------
// How many render passes we'll try before accepting the freshest one. Each pass
// after the first is compared to the previous; a matching pair wins early.
const MAX_CAPTURE_ATTEMPTS = 8;
// Pause between passes so pending image decodes in html-to-image's clone can
// finish. Generous on purpose — correctness over speed.
const CAPTURE_SETTLE_MS = 250;
// Downscaled comparison grid. Small enough to be cheap, large enough that a
// missing device frame or screenshot changes the signature.
const SIGNATURE_SIZE = 48;
// Max average per-byte difference (0–255) for two passes to count as identical.
// Non-zero to tolerate sub-pixel antialiasing jitter between renders.
const SIGNATURE_MATCH_TOLERANCE = 2;

function settle(ms: number): Promise<void> {
  return new Promise((resolve) => setTimeout(resolve, ms));
}

// Force every <img> inside the export node to fully decode before capture, so
// html-to-image's clone has real bitmaps to serialize instead of empty frames.
  ...
  await Promise.all(
    imgs.map(async (im) => {
      try {
        ...
        /* a broken image shouldn't block the whole export */
      }
    }),
  );
}

// Rasterize a PNG data URL down to a tiny grid so two capture passes can be
// compared cheaply. Returns null if the image can't be decoded.
async function captureSignature(dataUrl: string): Promise<Uint8ClampedArray | null> {
  try {
    const im = new Image();
    im.src = dataUrl;
    ...
    if (!ctx) return null;
    ctx.drawImage(im, 0, 0, SIGNATURE_SIZE, SIGNATURE_SIZE);
    return ctx.getImageData(0, 0, SIGNATURE_SIZE, SIGNATURE_SIZE).data;
  } catch {
    return null;
  }
}

// Two signatures match when they're byte-for-byte close. A null (undecodable)
// signature never matches, forcing another attempt.
function signaturesMatch(
  ...
  return total / a.length <= SIGNATURE_MATCH_TOLERANCE;
}
